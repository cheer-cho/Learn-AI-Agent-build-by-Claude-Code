"""Tests for the Module 05 reference solution.

Fully offline: every test uses the deterministic HashEmbeddingClient — no
model download, no network, no API key. The hash client has no semantics
(word overlap only), which is exactly enough to verify the plumbing these
tests care about: shapes, symmetry, ordering, and keyword behavior.
"""
...
lab = import_from_path("m05_solution_embeddings_lab", MODULE_DIR / "solution" / "embeddings_lab.py")
